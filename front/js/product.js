...
.then(data => {
    ...
    addToCart.addEventListener('click', () => {
        ...
        if(!newItem.color || !errorMessage) {
            itemColor.innerHTML += `<p class="errorMessage">Veuillez entrer une couleur</p>`
            console.log(errorMessage)
        }
        else  {
            console.log(newItem.color)
            errorMessage.innerText = 'dbdfbdfb'
        }
        
        if(itemQuantity.value < 1 || itemQuantity.value > 100 && !errorMessage) {
            let item = document.querySelector('.item__content__settings__quantity')
            item.innerHTML += `<p class="errorMessage">Veuillez entrer un nombre entre 1 et 100</p>`
            console.log('testt')
        }
